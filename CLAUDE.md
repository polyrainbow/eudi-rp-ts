...
- `test/fixtures/real/` is a genuine credential from the EU reference issuer, **expiring
  2026-11-08**, so `test/real-credential.test.ts` pins a fixed `now` and skips itself after that
  date. Committed private keys there are throwaway and documented in `test/fixtures/real/README.md`.
